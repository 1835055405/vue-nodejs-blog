    ...
    <div class="content">
      <div class="myInfo">
        <div class="myName">
          ...
          <span>我的签名：</span>
          <input type="text" v-model="signature" /><br />
          <span>我的密码：</span>
        ...
        res => {
          ...
          switch (res.code) {
            case "200":
              ...
              this.passowrd = info.passowrd;
              //如果一开始img不存在，那么就显示默认图片
              if (!this.imgsrc) {
                this.imgsrc = "/static/img/default-avatar-1716521282680.png";
              }
              break;
            case "400":
              console.log("获取信息错误！");
    ...
    changeInfo() {
      ...
      if (r) {
        if (!this.passowrd) {
          ...
          formData.append("password", this.passowrd);
          formData.append("signature", this.signature);
          formData.append("breif", this.breif);
          console.log(formData, "formdata");
          api.changeUserInfo(formData).then(
            res => {
              switch (res.code) {
                case "200":
                  console.log(res);
